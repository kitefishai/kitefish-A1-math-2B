...
EXTERNAL_DIR   = BASE_DIR / "external" / "val"
OUT_DIR = BASE_DIR / "dataset_train_val_200GB"


OUT_DIR.mkdir(parents=True, exist_ok=True)

BASE_DS = Path(IN_DIR) / "val.jsonl"
date_str = datetime.now().strftime("%Y%m%d")
OUTPUT_FILE = Path(OUT_DIR) / f"val_merged_{date_str}.jsonl"

# Sampling / repeat factors (based on your plan)
DATASETS = {
    "mathinstruct_val.jsonl": {
        "source": "mathinstruct",
        "repeat": 5,
        "text_key": "instruction",  # common format
        "answer_key": "output",
    },
    "pubmedqa_sciq_val.jsonl": {
    ...
    "openwebmath_5_val.jsonl": {
            "source": "openwebmath",
            "repeat": 1,
            "text_key": "text",
        },
    "stackexchange_1_val.jsonl": {
...
async def main():
    ...
    async for rec in read_jsonl(BASE_DS):
